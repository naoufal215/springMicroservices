...
public class MessageProcessorConfig {
	...
	public Consumer<Event<Integer, Review>> messageProcessor(){
		return event -> {
			LOG.info("Provess message created at {}...", event.getEventCreatedAt());
			
			switch(event.getEventType()) {
				
			case CREATE:
				Review review = event.getData();
				LOG.info("Creates review with ID: {} / {}", review.getProductId(), review.getReviewId());
				reviewService.create(review).block();
				break;
			case DELETE:
